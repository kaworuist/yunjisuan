  ...
  private shadeRay(ray: Ray): Colour {
    ...
    if(t > 0.0 && t < Number.MAX_VALUE) {
      ...
      for(let light of this.scene.lights) {
        ...
        let lightVect: vec4 = vec4.create();
        vec4.subtract(lightVect, light.pos, hit.intersection);
        let lightDist: number = vec4.length(lightVect);
        vec4.normalize(lightVect, lightVect);
        
        let minLight = hitObject.material.ka * this.scene.ambientLevel
        let lightIntensity: number = Math.max(minLight, vec4.dot(lightVect, hit.normal)) * light.brightness;
        ...
        let shadowRay: Ray = new Ray(hit.intersection, lightVect);
        ...
        if(!shadow) {
          // Specular Phong term
          let rv: number = Math.max(0.0, vec4.dot(hit.reflected, lightVect)); 
